...
float dsp::Grain::play() {
    ...
    if (m_position >= m_duration) {
        m_active = false;
        m_delay = static_cast<uint32_t>(static_cast<float>(m_duration) * static_cast<float>(rand() / static_cast<float>(RAND_MAX)));
    }
...
void dsp::Grain::setDuration(const float s) {
    {
        m_duration = std::max(static_cast<uint32_t>(20), static_cast<uint32_t>(s * static_cast<float>(m_size)));
    }
}
